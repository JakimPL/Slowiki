...
import { BEFORE_ANY_TURN, freshFrame, freshMarks, freshWaving } from "./fresh";
...
export function useFreshPlay(play: PlayRecord | null, mySeat: number | null, size: number): FreshHold {
    ...
    return {
        marks: freshMarks(play, mySeat, acknowledged),
        frame: freshFrame(play, size),
        waving: freshWaving(play, mySeat, acknowledged),
        acknowledge,
    };
}
