...
describe("Rack", () => {
    it("stays passive without a lift handler", () => {
        const markup = renderToStaticMarkup(<Rack tiles={TILES} liftedId={null} onLift={null} />);
        expect(markup).toContain('role="img"');
        expect(markup).not.toContain("<button");
    });

    it("offers each tile as a lift button and marks the lifted one", () => {
        const markup = renderToStaticMarkup(<Rack tiles={TILES} liftedId={2} onLift={() => undefined} />);
    ...
    it("names blank tiles for assistive tech", () => {
        ...
        const markup = renderToStaticMarkup(<Rack tiles={[blank]} liftedId={null} onLift={() => undefined} />);
