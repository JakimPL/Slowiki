...
def resolved(tmp_path: Path, locale: str, body: str) -> None:
    reference = read_catalogue(written(tmp_path, "en", REFERENCE))
    authored = read_catalogue(written(tmp_path, locale, body))
    resolved_catalogue(locale, authored, shape_of(reference))


def test_reads_nested_namespaces_as_dotted_keys(tmp_path: Path) -> None:
    catalogue = read_catalogue(written(tmp_path, "en", REFERENCE))
    assert sorted(catalogue.plain) == ["arrive.code_label", "seats.gathering"]
    assert sorted(catalogue.plural) == ["arrive.offering"]
    assert sorted(catalogue.plural["arrive.offering"]) == ["one", "other"]


def test_shape_carries_the_declared_placeholders(tmp_path: Path) -> None:
    shape = shape_of(read_catalogue(written(tmp_path, "en", REFERENCE)))
...
def test_emitted_templates_drop_the_type_declarations(tmp_path: Path) -> None:
    reference = read_catalogue(written(tmp_path, "en", REFERENCE))
    catalogue = resolved_catalogue("en", reference, shape_of(reference))
    assert catalogue.plain["seats.gathering"] == "{present} of {total}"


def test_english_plural_fills_every_category(tmp_path: Path) -> None:
    reference = read_catalogue(written(tmp_path, "en", REFERENCE))
    catalogue = resolved_catalogue("en", reference, shape_of(reference))
    entry = catalogue.plural["arrive.offering"]
...
def test_refuses_a_directory_without_the_reference(tmp_path: Path) -> None:
    written(tmp_path, "pl", REFERENCE)
    with pytest.raises(InvalidConfiguration, match="reference catalogue"):
...
def test_refuses_categories_that_disagree_on_placeholders(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConfiguration, match="different placeholders"):
        shape_of(read_catalogue(written(tmp_path, "en", body)))
...
def test_refuses_a_plural_message_holding_more_than_categories(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConfiguration, match="nothing beside its categories"):
        read_catalogue(written(tmp_path, "en", body))
...
def test_generation_reproduces_the_committed_sources(tmp_path: Path) -> None:
    main(CATALOGUE_DIR, tmp_path)
    expected = {"keys.ts", *(f"{locale}.ts" for locale in locale_names(CATALOGUE_DIR))}
